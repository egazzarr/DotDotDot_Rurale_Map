...
try:
    ...
    r"C:\Users\gazza\AppData\Local\Comfy-Desktop\ComfyUI-Shared\output"
)
except ImportError:
...
class SharpPredictGaussiansFromMetricDepth(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Float.Input(
                    "centre_density_percent",
                    default=100.0, min=1.0, max=100.0, step=1.0,
                    optional=True,
                    tooltip="Percentuale di Gaussiane mantenute al centro dell'ellissoide.",
                ),
                io.Float.Input(
                    "ellipsoid_quantile",
                    default=0.90, min=0.50, max=0.999, step=0.01,
                    optional=True,
                    tooltip="Dimensione dell'ellissoide. Valori minori eliminano più outlier.",
                ),
                io.Float.Input(
                    "density_falloff",
                    default=2.0, min=0.25, max=8.0, step=0.25,
                    optional=True,
                    tooltip="Velocità con cui la densità diminuisce verso il bordo.",
                ),
                io.Float.Input(
                    "point_scale",
                    default=0.003, min=0.0001, max=0.1, step=0.0001,
                    optional=True,
                    tooltip="Dimensione isotropica delle Gaussiane. Ridurre per punti più netti.",
                ),
            ],
    ...
    def execute(
        ...
        edge_crop: float = 0.0,
        centre_density_percent: float = 100.0,
        ellipsoid_quantile: float = 0.90,
        ...
        for b in range(B):
            ...
            # ------------------------------------------------------------------
            # 3D ellipsoidal density falloff
            # ------------------------------------------------------------------

            positions = gaussians.mean_vectors[0]  # [N, 3]

            # Centro robusto: la mediana non viene trascinata dagli outlier.
            centre = torch.median(positions, dim=0).values

            offset = positions - centre

            # Raggi robusti indipendenti per X, Y e Z.
            # Gli elementi oltre il quantile scelto finiranno fuori dall'ellissoide.
            q = float(ellipsoid_quantile)
            radii = torch.quantile(torch.abs(offset), q, dim=0)

            # Evita divisioni per zero se una dimensione è quasi piatta.
            radii = torch.clamp(radii, min=1e-5)

            # Distanza ellissoidale 3D:
            # r=0 al centro, r=1 sulla superficie dell'ellissoide.
            ellipsoid_distance = torch.sqrt(
                torch.sum((offset / radii) ** 2, dim=1)
            )

            # Densità massima regolabile al centro e graduale verso zero.
            centre_probability = float(centre_density_percent) / 100.0

            keep_probability = (
                centre_probability
                * torch.clamp(1.0 - ellipsoid_distance, min=0.0, max=1.0)
                .pow(float(density_falloff))
            )

            keep_mask = torch.rand_like(keep_probability) < keep_probability

            # Assicura che non venga prodotto un PLY vuoto.
            if not torch.any(keep_mask):
                keep_mask[torch.argmin(ellipsoid_distance)] = True
            ...
            # ------------------------------------------------------------------
            # Convert large anisotropic splats into small isotropic "points"
            # ------------------------------------------------------------------

            fixed_scale = torch.full_like(
                gaussians.singular_values,
                float(point_scale),
            )

            gaussians = gaussians._replace(
                singular_values=fixed_scale,
            )
            ...
            # Make the PLY accessible to Preview Gaussian Sharp
            PREVIEW_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
            preview_path = PREVIEW_OUTPUT_DIR / Path(ply_path).name

            if Path(ply_path).resolve() != preview_path.resolve():
                shutil.copy2(ply_path, preview_path)
